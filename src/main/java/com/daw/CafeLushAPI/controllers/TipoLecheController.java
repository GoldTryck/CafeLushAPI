...
@RestController
@RequestMapping("/api/tipo-leche")
@RequiredArgsConstructor
public class TipoLecheController {

    private final ComplementoService<TipoLeche> tipoLecheService;

    @PostMapping
    public ResponseEntity<ComplementoDTO> crear(@RequestBody @Valid ComplementoDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(tipoLecheService.crear(dto));
    }

    @GetMapping
    public List<ComplementoDTO> obtenerTodos() {
        return tipoLecheService.obtenerTodos();
    }
}
